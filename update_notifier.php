...
function update_notifier_menu() {  
	...
	if( function_exists( 'wp_get_theme' ) ) {
		...
		if(version_compare($theme_obj->get('Version'), $xml->latest) == -1) {
		add_dashboard_page( $theme_obj->get('Name') . 'Theme Updates', $theme_obj->get('Name') . '<span class="update-plugins count-1"><span class="update-count">New Updates</span></span>', 'administrator', strtolower($theme_obj->get('Name')) . '-updates', 'update_notifier' );
		}
	} else {
...
function get_latest_theme_version($interval) {
	...
	$notifier_file_url = 'http://billyf.expertvm.com/Matrix/notifier.xml';
